...
float4 GaussianBlur(Texture2D tex2D,SamplerState state,float3 uv)
{
    float smp = tex2D.Sample(state,uv) * weight[0];
    for (int i=1; i<3; i++) 
	{
        smp += tex2D.Sample(state,uv + float2(0.0,offset[i])) * weight[i];
        smp += tex2D.Sample(state,uv - float2(0.0,offset[i])) * weight[i];
    }
	return smp;
}
...
float4x4 AxisAngle4x4(float3 axis, float angle)
{
    ...
    float s = sin(angle);
    float c = cos(angle);
    float oneminVus_c = 1.0 - c;

    axis = normalize(axis);
    float4x4 rotMatrix = 
    {   oneminVus_c * axis.x * axis.x + c, oneminVus_c * axis.x * axis.y - axis.z * s, oneminVus_c * axis.z * axis.x + axis.y * s, 0,
        oneminVus_c * axis.x * axis.y + axis.z * s, oneminVus_c * axis.y * axis.y + c, oneminVus_c * axis.y * axis.z - axis.x * s, 0,
        oneminVus_c * axis.z * axis.x - axis.y * s, oneminVus_c * axis.y * axis.z + axis.x * s, oneminVus_c * axis.z * axis.z + c, 0,
        0                                         , 0                                         , 0                                , 1,
    };
...
float3x3 AxisAngle3x3(float3 axis, float angle)
{
    ...
    axis = normalize(axis);
    float3x3 rotMatrix = 
    {   oneminVus_c * axis.x * axis.x + c, oneminVus_c * axis.x * axis.y - axis.z * s, oneminVus_c * axis.z * axis.x + axis.y * s,
        oneminVus_c * axis.x * axis.y + axis.z * s, oneminVus_c * axis.y * axis.y + c, oneminVus_c * axis.y * axis.z - axis.x * s,
        oneminVus_c * axis.z * axis.x - axis.y * s, oneminVus_c * axis.y * axis.z + axis.x * s, oneminVus_c * axis.z * axis.z + c
    };
