...
public class ShapeTrailRenderer : MonoBehaviour
{
    ...
    void PointBehavior(int id)
    {
        ...
        if(id>0)
        {
            trail[id].velocity += force * Time.deltaTime;
            trail[id].velocity -= (trail[id].position-transform.position) * spring * Time.fixedDeltaTime;
            trail[id].velocity -= trail[id].velocity * damp * Time.fixedDeltaTime;
            trail[id].position += trail[id].velocity;
